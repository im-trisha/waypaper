...
class Config:
    ...
    def __init__(self):
        ...
        self.image_folder_list: list[Path] = []
        ...
        self.fill_option = config.get("Settings", "fill", fallback=FillOptions.FILL.value)
        self.sort_option = SortOptions.NAME
        ...
        self.swww_transition_type = SwwwTransitionTypes.ANY
        ...
        if wallpapers_str:
            self.wallpapers = [
                Path(paper).expanduser() for paper in wallpapers_str.split(",")
            ]

    def select_wallpaper(self, path_str: str) -> None:
        self.selected_wallpaper = Path(path_str)
    ...
    def get_image_folder_list(self, section: str, config: ConfigParser) -> list[Path]:
        image_folders_str: list[str] = config.get(
            section, "folder", fallback=self.image_folder_fallback
        ).split("\n")
        image_folder_list = [
            Path(path_str).expanduser() for path_str in image_folders_str
        ]
    ...
    def read_parameters_from_user_arguments(self, args: Namespace) -> None:
        ...
        if args.folder:
            self.image_folder_list = [Path(path_str) for path_str in args.folder]
        if args.state_file:
